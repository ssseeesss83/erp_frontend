      ...
      <div style="width: 90%; margin: 0 auto">
        <el-form :model="temp" label-width="100px" ref="temp">
          <el-form-item label="进货单id: " prop="purchaseSheetId">
            <el-select v-model="temp.purchaseSheetId"
          ...
          <el-form-item label="进货单清单: ">
            <div
                v-for="(item, index) in temp.purchaseReturnsSheetContent"
                :key="index">
              <el-row>
                <el-col :span="8">
...
<script>
import {getAllPurchase, returnFirstApproval, returnSecondApproval} from '@/network/purchase'
import {purchaseReturnCopyIn, purchaseReturnRedFlush} from "@/network/financial";
export default {
  ...
  methods: {
    redFlush(it){
      console.log(it)
      purchaseReturnRedFlush(it).then(res=>{
        this.$message({
          message: '操作成功!',
          type: 'success'
        })
    ...
    copyIn(it){
      purchaseReturnRedFlush(it).then(res=>{
        ...
        purchaseReturnCopyIn(it).then(_res=>{
          this.$message({
            message: '操作成功!',
            type: 'success'
          })
