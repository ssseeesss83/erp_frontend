    ...
    <el-card v-for="(item, index) in list" :index="item.index" :key="item.id" shadow="hover">
      ...
      <div>
        ...
        <div v-if="showAll[index]" style="margin-top: 15px">
          ...
          <el-table
              :data="item.payableSheetContentVOS"
              stripe
              style="width: 100%"
    ...
    <el-dialog
      ...
      <div style="width: 90%; margin: 0 auto">
        <el-form :model="temp" label-width="100px" ref="temp">
          ...
          <el-form-item
              v-for="(item, index) in temp.payableSheetContentVOS"
              :key="index"
              :label="'账户' + index">
            <div>
            ...
            <div style="margin-top: 10px">
              <el-button type="text" size="small" @click="addList" v-if="index === temp.payableSheetContentVOS.length - 1">添加</el-button>
...
<script>
import {approvePayable, getAllAccount, payableCopyIn, payableRedFlush} from '@/network/financial'
...
export default {
  ...
  methods: {
    ...
    approval(id) {
      ...
      approvePayable(config).then(res =>{
        this.$emit("refresh")
        this.$message({
          message: '操作成功!',
          type: 'success'
